...
if (Platform.OS !== 'web') {
  try {
    ...
    // WebRTC not available - silently fail
  }
}
...
class WebRTCService {
    ...
    this.socket.on('call:ice-candidate', async ({ candidate }) => {
      if (candidate && this.peerConnection) {
        try {
          ...
          // Silently fail - ICE candidate errors are handled by recovery mechanisms
        }
      }
    });
  ...
  async startCall(callData: CallData) {
    ...
    try {
      ...
      // Create peer connection with appropriate config
  ...
  private startCallQualityMonitoring() {
    ...
    this.qualityMonitorInterval = setInterval(async () => {
      ...
      try {
        ...
        // Silently fail - stats errors are not critical
      }
