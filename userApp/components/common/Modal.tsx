...
export const Modal: React.FC<ModalProps> = ({
  ...
  const renderButtons = () => {
    if (buttons && buttons.length > 0) {
      // Only stack vertically if more than 2 buttons
      // For 1-2 buttons, always use horizontal layout for better UX
      if (buttons.length > 2) {
        return (
...
const styles = StyleSheet.create({
  ...
  actionButton: {
    ...
    minWidth: getResponsiveSpacing(120, 130, 140),
    alignItems: 'center',
    justifyContent: 'center',
    flex: 1,
    maxWidth: '48%', // Allow buttons to take up to 48% each, leaving space for gap
    minHeight: getResponsiveSpacing(44, 48, 52), // Ensure proper touch target
  },
