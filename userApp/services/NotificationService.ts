...
class NotificationService {
  ...
  private async registerTokenWithBackend(token: string): Promise<boolean> {
    try {
      // Try to get token using tokenManager if available, otherwise use AsyncStorage
      ...
      try {
        ...
        // Fallback to AsyncStorage if tokenManager is not available
      ...
      const deviceInfo = {
        ...
        appType: 'user', // Distinguish from providerApp
      };
